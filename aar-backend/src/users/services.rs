...
pub async fn get_user_by_email(my_pool: &Rbatis, email: &str) -> GqlResult<User> {
    ...
    if user.is_ok() {
        ...
        Err(Error::new("email 不存在").extend_with(|_, e| e.set("details", "1_EMAIL_NOT_EXISTS")))
    }
}

// 插入新用户
pub async fn new_user(my_pool: &Rbatis, mut new_user: NewUser) -> GqlResult<User> {
    new_user.email = new_user.email.to_lowercase();

    if self::get_user_by_email(my_pool, &new_user.email)
        .await
        .is_ok()
    {
